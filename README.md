...
A Burp Suite extension that generates TOTP/HOTP two-factor authentication codes,
so you can log in to 2FA-protected accounts without leaving Burp.

## Features

- **Live codes** for every registered account, refreshed continuously with an expiry indicator.
- **Register from `otpauth://` URIs** - paste one or several at once.
- **Register from QR codes** - pick an image file or paste an image from the clipboard.
- **Register from HTTP responses** - right-click a selected response (Proxy history, Site map,
  message editor) and choose *Register 2FA codes from response*. QR code images,
  `otpauth://` URIs in JSON/HTML and `data:image/...;base64` images are all detected.
- **Google Authenticator exports** - `otpauth-migration://offline?data=...` payloads are
  decoded into all contained accounts.
- **Copy codes** with a click, double-click, Ctrl/Cmd+C or the toolbar button.
- **HOTP support** including counter increment.
- **Share progress with teammates** - export registered accounts to a file and import them
  elsewhere. Exports default to passphrase-encrypted JSON; plaintext JSON and a plain
  `otpauth://` URI list are also available for interoperability.

## Usage

1. Build the JAR (see [Building your extension](#building-your-extension)).
2. Load it into Burp and open the **2FA Codes** suite tab.
3. Use **Add** to register accounts from a URI, a QR image, or the clipboard. During a
   pentest, right-click a response containing the enrolment QR code and choose
   **Register 2FA codes from response** instead.
4. Click a row (or double-click) to copy the current code.
5. Use **Export…** to share accounts and **Import…** to load a teammate's export. Share the
   file and its passphrase over separate channels. Re-importing skips accounts you already
   have, but advances a local HOTP counter if the incoming one is further along.

> **Secret storage:** shared secrets are stored unencrypted in the Burp project file by
> default so they survive restarts. Uncheck *Remember secrets in the project file* to keep
> them in memory only.

> **Export security:** the encrypted export uses PBKDF2-HMAC-SHA256 (600,000 iterations)
> and AES-256-GCM, so a tampered file fails to decrypt instead of importing modified
> secrets. Plaintext exports print the secrets in the clear - only use them when the
> encrypted option is not workable.
